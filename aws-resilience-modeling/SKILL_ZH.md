...
3. **检查配置参数**：确认已安装 MCP 的 `AWS_REGION` 和 `AWS_PROFILE` 是否与用户本次评估的目标账户/区域匹配
4. **处理结果**：
   - **必需 MCP 缺失**：向用户展示缺失的 MCP 列表，并提供安装命令（参见 [MCP_SETUP_GUIDE_zh.md](references/MCP_SETUP_GUIDE_zh.md)），等待用户安装后再继续
   - **MCP 已安装但配置不匹配**（如 Region/Profile 与目标不同）：提示用户需要重新配置或新增针对目标环境的 MCP 实例
   - **已有非本 Skill 要求的 MCP**：忽略不相关的 MCP，仅使用本 Skill 要求的 AWS MCP 服务器进行扫描
   - **必需 MCP 均已正确配置**：直接进入下一步

> **重要**：不要直接使用用户系统中已有的非 AWS 官方 MCP 服务器进行资源扫描。必须使用上方列表中指定的 `awslabs.*-mcp-server` 系列服务器。
